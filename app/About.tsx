    ...
    <div className="w-full bg-neutral-50">
      <div id="about" className="mx-auto lg:flex gap-12 py-24 px-12 lg:max-w-7xl lg:px-8">
        <div className="lg:my-0 basis-7/12">
          <h2 className="text-3xl font-semibold tracking-tight text-title-gray sm:text-4xl font-serif">About The Project</h2>
          ...
          <dl className="mt-16 grid grid-cols-1 gap-x-6 gap-y-10 lg:grid-cols-2 sm:gap-y-16 lg:gap-x-8">
            {features.map((feature) => (
              <div key={feature.name} className="border-t border-gray-200 pt-4">
                <dt className="text-title-gray font-bold font-serif">{feature.name}</dt>
                <dd className="mt-2 text-sm text-gray-500 font-light">{feature.description}</dd>
              </div>
            ))}
          </dl>
        </div>
